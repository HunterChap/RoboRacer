...
def generate_launch_description():
    ...
    launch_arguments = [
        ...
        DeclareLaunchArgument(
            ...
            default_value='true',
            description=(
                'Start the ROS 2 game-controller driver and controller input '
                'node. The final priority mux always runs.'
            ),
        ),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            ...
            default_value='false',
            description=(
                'Enable physical VESC and steering output. Keep false until '
                'vehicle_driver_node hardware communication is implemented '
                'and calibrated.'
            ),
        ),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            'controller_max_speed_mps',
            default_value='0.35',
            description='Maximum controller speed for initial real-car tests.',
        ),
    ]
    ...
    # This mux always runs. With controller input disabled, it simply forwards
    # AEB-filtered terminal/Auto commands to the final safe command topic.
    ...
    # Mirror the final safe Twist command into two Ackermann outputs.
    # The real branch publishes /drive_target for vehicle_driver_node.
    # The simulator branch remaps that output to /drive.
    real_cmd_vel_to_ackermann_node = Node(
        package='roboracer_py',
    ...
    sim_cmd_vel_to_ackermann_node = Node(
        package='roboracer_py',
        executable='cmd_vel_to_ackermann_node',
